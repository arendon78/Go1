...
coordinates_considered  = fl_coordinates # Tried to change the foot considered to see if pattern matching would work  : it doesn't work. 
#You have t to change the parameters of the filter again for it to work properly.




times = times = list(range(len(coordinates_considered)))

#-------------- to decomment once finished
local_maxes = monte_carlo_gradient(1,coordinates_considered)

#---------------- to decomment once finished

local_mins = monte_carlo_gradient(-1,coordinates_considered)
...
find_patterns(merged_mins_maxes_coordinates,coordinates_considered, 700,700,5.5)
...
axs[0].plot(times, [fr_coordinates[i][1] for i in range (len(rr_coordinates))], label='FL', marker='o')
axs[0].set_title('FR Forces Over Time')
axs[0].set_ylabel('Force')
axs[0].legend()

# axs[1].plot(times, fl_forces, label='FL', marker='o')
axs[1].plot(times, [coordinates_considered[i][1] for i in range (len(coordinates_considered))], label='FR', marker='o')
axs[1].plot(times, [filled_fr_maxes[i][1] for i in range(len(filled_fr_maxes))], label='FR maxes', marker='o')
axs[1].plot(times, [filled_fr_mins[i][1] for i in range(len(filled_fr_mins))], label='FR mins', marker='o')
